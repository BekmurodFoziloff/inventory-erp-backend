...
export class AttributesService {
  ...
  private readonly defaultPopulate = [{ path: 'values', select: 'name' }];
  ...
  async getLookup(): Promise<Attribute[]> {
    return this.attributeModel
      .find({ deletedAt: null, isActive: true }, 'name')
  ...
  async create(createAttributeDto: CreateAttributeDto): Promise<Attribute> {
    try {
      ...
      return populated.toObject({ virtuals: true }) as any as Attribute;
    } catch (error) {
      throw error;
    }
  }
